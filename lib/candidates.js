  ...
  request.get({
    ...
    if( !err && res.statusCode === 200 ) {
      /*
        ...
        Note: we have to check ourselves as the events platform
              does not support checking for events before a date yet.
       */
      events = JSON.parse( eventsJSON );
      events = events.filter( function( event ) {
        return ( (new Date( event.beginDate )) < (new Date( env.get( 'events_before' ) )) );
      });
...
module.exports = {
  ...
  getCandidateDetails: function( username ) {
    if( !userDetails[ username ] ) {
      return {};
    }

    return JSON.parse( JSON.stringify( userDetails[ username ] ) );
  }
};
